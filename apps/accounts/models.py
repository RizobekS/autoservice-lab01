...
class UserManager(BaseUserManager):
    def _create_user(self, email, first_name, raw_password, save=False):
        if not first_name:
            raise ValueError('Имя не указано')
        if not email:
            raise ValueError('Эл. почта не указана')
        email = self.normalize_email(email)
        user = self.model(first_name=first_name, email=email, username=email)
    ...
    def create_user(self, email, first_name, password, **other_fields):
        user = self._create_user(email, first_name, password, save=True)
        return user

    def create_superuser(self, email, first_name, password, **other_fields):
        user = self._create_user(email=email, first_name=first_name, raw_password=password, save=False)
        user.is_admin = True
        user.is_staff = True
...
class User(AbstractUser):
    ...
    is_active = models.BooleanField('Активен', help_text='Вместо удаления пользователя просто снимите этот флажок', default=True)
