...
(function($) {
    ...
    function initChart(el) {
        ...
        var units = data.units ? data.units : 'percent';

        switch (units) {
            case 'month':
                {
                    var onStep = function(from, to, percent) {
                        $(this.el).find('.percent').text(Math.round(percent / 100 * 12));
                    }
                    break;
                }
            default:
    ...
    function documentReadyInit() {
        ...
        $('form.searchform, form.search-form').on('submit', function(e) {
            ...
            //sending form data to PHP server if fields are not empty
            var request = $form.serialize();
            // Search is done by another js query
        ...
        var filterSelects = document.querySelectorAll('.choices-plain');
        ...
        if (document.querySelector('.float-socials')) {
            var floatBlock = new StickySidebar('.float-socials', {
                containerSelector: '.entry-content-aside',
                innerWrapperSelector: '.ya-share2',
                resizeSensor: true,
                topSpacing: 70,
                //   bottomSpacing: 20
            });
    ...
    function windowLoadInit() {
        ...
        if ($().flexslider) {
            ...
            $introSlider.each(function(index) {
                ...
                $currentSlider.flexslider({
                    ...
                    start: function(slider) {
                        ...
                        slider.find('.flex-active-slide').each(function(index) {
                            var self = $(this);
                            console.log(self);
                            var bg = self.data('sliderBg');
                            console.log('bg: ',bg);
                            if (bg) {
                                self.css('backgroundImage','url(' + bg + ')');
                            }
                        });
                    },
                    after: function(slider) {
                        console.log('flex slider after');
                        slider.find('.flex-active-slide .intro_layers').children().each(function(index) {
                            var self = $(this);
                            var animationClass = !self.data('animation') ? 'fadeInRight' : self.data('animation');
                            setTimeout(function() {
            ...
            if (document.documentElement.clientWidth <= 767) {
                ...
                $(".nav-tabs-slider-mobile").each(function(index) {
                    var $currentSlider = $(this);
                    if ($currentSlider.find('.flex-active-slide').length) {
                        return;
                    }
                    $currentSlider.flexslider({
                        ...
                        controlNav: false,
                        after: function(slider) {
                            $(slider.slides[slider.currentSlide]).find('a').tab("show");
                        }
                    });
                });
            }
    ...
    if (typeof(Typed) !== undefined && $('.typed').length) {
        ...
        $('.typed').each( function(index) {
            console.log($(this).find('#typed-strings'));

            typed[index] = new Typed(this, {
                stringsElement: '#typed-strings'+index,
                typeSpeed: 100,
                backSpeed: 50,
                backDelay: 500,
                startDelay: 1000,
